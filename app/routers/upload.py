...
from app.models import CaseBatch, CaseFile, EventType, FileStatus, PiiEntity, SweepStatus, User, UserRole
...
from app.services.security_sweep_service import security_sweep_service
...
def _bot_guard(db: Session, user: User, bureau_field: str, request: Request) -> None:
    if bureau_field != "":
        create_audit_log(
            db,
            event_type=EventType.bot_submission_detected,
            user_id=user.id,
            metadata={"ip": request.client.host if request.client else "", "user_agent": request.headers.get("user-agent", "")},
        )
        db.commit()
...
def _process_file(
    *,
    ...
    for skipped_entity in skipped[:100]:
        create_audit_log(
            db,
            event_type=EventType.context_link_skipped,
            user_id=user.id,
            file_id=case.id,
            metadata={
...
async def _process_upload_file(
    *,
    ...
    existing = (
        db.query(CaseFile)
        .filter(CaseFile.file_hash == file_hash, CaseFile.owner_id == user.id, CaseFile.expires_at > datetime.utcnow())
        .first()
    )
    if existing:
        ...
        db.commit()
        return {"duplicate": True, "case_id": existing.id, "message": "Case already exists and is active"}

    sweep = security_sweep_service.check_hash(file_hash)
    if sweep.status == "QUARANTINED":
        quarantined_case = CaseFile(
            ...
            sweep_status=SweepStatus.quarantined,
            uploaded_by=user.id,
            owner_id=user.id,
            batch_id=batch_id,
            file_hash=file_hash,
        ...
        create_audit_log(
            db,
            event_type=EventType.case_quarantined,
            user_id=user.id,
            file_id=quarantined_case.id,
            metadata={"file_hash": file_hash, "vt_report": sweep.report},
        )
        db.commit()
        return {"file": upload.filename, "status": FileStatus.quarantined.value, "case_id": quarantined_case.id}

    sweep_status = SweepStatus.pending if sweep.status == "PENDING" else SweepStatus.passed
    if sweep_status == SweepStatus.passed:
        create_audit_log(db, event_type=EventType.security_sweep_passed, user_id=user.id, metadata={"file_hash": file_hash, "vt": sweep.report})
    else:
        create_audit_log(db, event_type=EventType.security_sweep_failed, user_id=user.id, metadata={"file_hash": file_hash, "vt": sweep.report})
    ...
    return {"file": Path(upload.filename).name, "case_id": case.id, "status": case.status.value, "risk_score": case.risk_score, "pii_count": case.pii_count}
...
async def upload_batch(
    ...
    batch = CaseBatch(name=name or f"Batch - {datetime.utcnow().isoformat()}", operative_id=user.id, total_files=len(uploads), completed_files=0)
    db.add(batch)
    db.commit()
    db.refresh(batch)
    ...
    for upload in uploads:
        try:
            if Path(upload.filename).suffix.lower() == ".zip":
                ...
                completed += sum(1 for r in zip_results if r.get("status") in {FileStatus.sealed.value, FileStatus.quarantined.value} or r.get("duplicate"))
            else:
                outcome = await _process_upload_file(db=db, user=user, upload=upload, masking_mode=masking_mode, batch_id=batch.id)
                results.append(outcome)
                if outcome.get("status") in {FileStatus.sealed.value, FileStatus.quarantined.value} or outcome.get("duplicate"):
                    completed += 1
        except Exception as exc:
            db.rollback()
            results.append({"file": upload.filename, "status": "failed", "error": str(exc)})
...
def batch_status(batch_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if user.role != UserRole.admin and batch.operative_id != user.id:
        raise HTTPException(status_code=404, detail="Batch not found")
    ...
    return {
        ...
        "files": [{"case_id": f.id, "status": f.status.value, "file_type": f.file_type} for f in files],
    }
